...
class RepositorioLanzamientos(ABC):
    """
    Clase abstracta que define la interfaz para el repositorio de lanzamientos.
    ...
    @abstractmethod
    def obtener_lanzamiento_por_id(
        self, id_lanzamiento: str
...
class RepositorioPostgresLanzamientos(RepositorioLanzamientos):
    ...
    def obtener_lanzamiento_por_id(
        ...
        lanzamiento = (
            self.db.query(ModeloLanzamiento)
            .filter(ModeloLanzamiento.id_lanzamiento == id_lanzamiento)
            .first()
        )
    ...
    def obtener_proximos_lanzamientos(self) -> List[Dict[str, Any]]:
        lanzamientos = (
            self.db.query(ModeloLanzamiento)
            .filter(ModeloLanzamiento.proximo == True)
            .all()
        )
        return [lanzamiento.a_dict() for lanzamiento in lanzamientos]
    ...
    def obtener_lanzamientos_pasados(self) -> List[Dict[str, Any]]:
        lanzamientos = (
            self.db.query(ModeloLanzamiento)
            .filter(ModeloLanzamiento.proximo == False)
            .all()
        )
        return [lanzamiento.a_dict() for lanzamiento in lanzamientos]
